...
func safe_get(name):
	...
	if (cache!=null):
		if (cache is Node):
			if (cache.is_queued_for_deletion() == false):

				return cachemap.get(name)
				
	safe_get_node(name)
func safe_get_node(name):

	if root.get_child(3).has_node(name):

		var node = root.get_child(3).get_node(name)
		cachemap[name] = node
		return node
	else:
		return false
...
func _process(delta):
	if root == null:
		...
		if config["wideScreen"]:
			...
			var szX = float(sz[0])
			var szY = float(sz[1])
			var mul = 540.0/(540.0*config["renderScale"])
